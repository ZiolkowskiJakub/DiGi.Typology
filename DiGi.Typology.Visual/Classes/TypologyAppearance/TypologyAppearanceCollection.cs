...
{
    /// <summary>
    /// A collection of <see cref="TypologyAppearance"/>, one per bucket, filed under the key <see cref="Query.Key(object)"/>
    ...
    /// <see cref="UniqueValueRuleData"/> buckets them; see <see cref="Query.Key(object)"/> for the forms, the range
    /// form included. Two limitations are inherent: a text value that is literally <c>"null"</c> shares the key of the
    /// NULL bucket, and floating-point keys are not guaranteed across a .NET Framework host, whose
    /// <c>double.ToString</c> differs - use a range rule for floating-point columns.</para>
    /// </summary>
    public class TypologyAppearanceCollection : SerializableObject, ITypologyVisualSerializableObject
    {
        ...
        /// <para>The value is normalised through <see cref="Query.Key(object)"/>, so any value type is accepted: <c>null</c>
        ...
        /// </summary>
        /// <param name="object">The value or range, its string form, or the rule data produced for it.</param>
        ...
        public TypologyAppearance? this[object? @object]
        {
            get
            {
                if (!appearances.TryGetValue(Query.Key(@object), out TypologyAppearance? result))
                {
                    return null;
                }
        ...
        {
            return appearances.ContainsKey(Query.Key(@object));
        }
    }
}